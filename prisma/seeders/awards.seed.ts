    ...
    try {
        ...
        for (const contest of contests) {
            ...
            const awardTypes = [
                ...
                AwardType.fourthPrize,
                AwardType.impressiveVideo,
                AwardType.excellentVideo
            ];
            ...
            for (const awardType of awardTypes) {
                ...
                if (contestants.length > 0 && Math.random() > 0.3) {
                    const contestContestants = contestants.filter(c => c.contestId === contest.id);

                    if (contestContestants.length > 0) {
                        const randomContestant = contestContestants[
                            Math.floor(Math.random() * contestContestants.length)
                        ];

                        if (!randomContestant) {
                            throw new Error(`Không thể tìm thấy thí sinh cho giải thưởng ${getAwardName(awardType)} trong cuộc thi ${contest.name}`);
                        }
...
function getAwardName(awardType: AwardType): string {
    switch (awardType) {
        ...
        case AwardType.thirdPrize:
            return "Giải Ba";
        case AwardType.fourthPrize:
            return "Giải Tư";
        case AwardType.impressiveVideo:
            return "Giải Video Ấn tượng";
        case AwardType.excellentVideo:
            return "Giải Video Xuất sắc";
        default:
