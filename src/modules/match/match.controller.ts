...
export default class MatchController {
  ...
  //     }
  //     logger.info(`Lấy danh sách trận đấuthành công`);
  //     res.json(
  //       successResponse(
  //         { rounds: data.rounds, pagination: data.pagination },
  //         "Lấy danh sách trận đấuthành công"
  //       )
  //     );
  //   } catch (error) {
  ...
  //     res.json(
  //       successResponse(round, `Thêm trận đấu${input.name} thành công`)
  //     );
  //   } catch (error) {
  //     logger.error((error as Error).message);
  //     res.status(400).json(errorResponse((error as Error).message));
  ...
  //     });
  //     if (!updateRound) {
  //       throw new Error("Cập nhật trạng thái trận đấuthất bại ");
  //     }
  //     logger.info(`Cập nhật trạng thái trận đấu ${round.name} thành công`);
  //     res.json(
  //       successResponse(
  //         updateRound,
  //         `Cập nhật trạng thái trận đấu${round.name} thành công`
  //       )
  //     );
  //   } catch (error) {
  //     logger.error((error as Error).message);
  ...
  //     if (!deleteRound) {
  //       throw new Error(`Xóa trận đấu${Round.name} thất bại `);
  //     }
  //     logger.info(`Xóa trận đấu${Round.name} thành công`);
  //     res.json(successResponse(null, `Xóa trận đấu${Round.name} thành công`));
  //   } catch (error) {
  //     logger.error((error as Error).message);
  //     res.status(400).json(errorResponse((error as Error).message));
  //   }
  ...
  //         messages.push({
  //           status: "error",
  //           msg: `Xóa trận đấu"${round.name}" thất bại`,
  //         });
  //         continue;
  //       }
  //       messages.push({
  //         status: "success",
  //         msg: `Xóa trận đấu"${round.name}" thành công`,
  //       });
  //       logger.info(`Xóa trận đấu"${round.name}" thành công`);
  //     }
  //     res.json({
