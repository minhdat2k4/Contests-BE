...
export const registerUpdateStatusByAdminEvents = (
  ...
  socket.on("contestant:status-update", async (rawData: unknown, callback) => {
    ...
    try {
      ...
      const data = await GroupDivisionService.getContestantByJudgeIdAndMatchId(
        payload.userId,
        match.id
      );
      ...
      io.of("/match-control").to(judgeRoom).emit("contestant:status-update", {
        match,
        data,
      });
